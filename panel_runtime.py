...
Runs the real panel callback runtime and keeps Music Intelligence v7 and
Coherence Intelligence v1 enabled.
"""
...
coherence_intelligence.apply(bot)
...
async def preview(update, context):
    ...
    try:
        ...
        if track:
            await target.reply_audio(audio=track["file_id"], caption=bot._audio_caption(track), title=track.get("title"), performer=track.get("performer"))
    except Exception as exc:
        bot.log.exception("Panel Preview send failed")
        await target.reply_text(f"❌ ارسال Preview شکست خورد: {exc}")
        return
    state = bot.load_state(); mood = package["mood"]
    ...
    bot._advance_mood(state, mood); bot.save_state(state)
...
async def callback(update, context):
    ...
    try:
        if action == "preview":
            ...
            await bot.post_once(context, True); await target.reply_text("✅ دستور Post اجرا شد.")
        elif action == "pause":
            state = bot.load_state(); state["paused"] = True; bot.save_state(state); await target.reply_text("⏸ زمان‌بندی متوقف شد.")
        elif action == "resume":
            state = bot.load_state(); state["paused"] = False; bot.save_state(state); await target.reply_text("▶️ زمان‌بندی فعال شد.")
        elif action == "stats":
            x = bot.DB.summary(); await target.reply_text("📊 SilentRuins\n" f"پست موفق: {x['posts']}\n" f"خطا: {x['failed']}\n" f"۲۴ ساعت اخیر: {x['today']}\n" f"میانگین کیفیت: {x['avg_quality']}/100\n" f"آهنگ‌ها: {x['tracks']}")
        elif action == "songs":
            ...
            if not tracks: await target.reply_text("🎵 کتابخانه خالی است.")
                ...
                for i, track in enumerate(tracks[-50:], 1):
                    mood = bot.MOODS.get(track.get("mood"), {}).get("fa", track.get("mood", "?")); lines.append(f"{i}. {track.get('performer', '?')} — {track.get('title', '?')} [{mood}]")
                await target.reply_text("\n".join(lines))
        elif action == "schedule":
            state = bot.load_state(); status = "⏸ متوقف" if state.get("paused") else "▶️ فعال"; await target.reply_text(f"🕐 Schedule: {', '.join(bot.POST_TIMES)}\n🌍 {bot.TZ_NAME}\n{status}\nThreshold: {bot.QUALITY_THRESHOLD}")
        elif action == "backup":
            stamp = dt.datetime.now(bot.TZ).strftime("%Y%m%d_%H%M%S"); path = bot.BACKUP_DIR / f"silentruins_{stamp}.zip"; bot.save_library(bot.load_library()); bot.save_state(bot.load_state())
        ...
        bot.log.exception("Panel callback failed: %s", data); await target.reply_text(f"❌ اجرای دکمه {data} با خطا مواجه شد: {exc}")
