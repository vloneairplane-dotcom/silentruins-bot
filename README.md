...
**متن احساسی ساده + استیکر 🎭 + آهنگ 🎧** (با اسم خواننده و ترک توی کپشن آهنگ)

## ✨ امکانات

- 📝 **پست متنی ساده** (پیش‌فرض، بدون عکس) — ۷۰+ متن فارسی طبیعی که ربات روشون می‌چرخه
...
- 📸 حالت عکس‌دار هم موجوده: `SEND_PHOTOS=true` → عکس دارک از Pexels اضافه می‌شه

## 📦 شکل هر پست

```
① متن ساده (مثل «یه جایی وسط روز، بی‌دلیل دلتنگ شدم.» + گاهی هشتگ + آیدی چنل)
...
| `PEXELS_API_KEY` | فقط اگه `SEND_PHOTOS=true` — از [pexels.com/api](https://www.pexels.com/api/) رایگان |
...
| `SEND_PHOTOS` | `false` | `true` کن تا پست‌ها عکس Pexels هم داشته باشن |
| `PEXELS_API_KEY` | — | فقط برای حالت عکس‌دار |
