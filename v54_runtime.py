"""SilentRuins v5.5 runtime.
Soundtrack-first matching: never sacrifice a strong mood match merely for preview diversity.
"""
...
def main():
    ...
    def labels_for(raw):
        text=norm(raw); labels=set()
        for label,moods in TAG_TO_MOODS.items():
            if text==label or label in text: labels.update(moods)
        return labels

    def music_score(track,mood):
        if not track:return 0.0
        tag=norm(track.get("mood")); title_artist=norm(f"{track.get('title','')} {track.get('performer','')}")
        if tag==mood:return 1.0
        if mood in labels_for(tag):return .96
        if mood in labels_for(title_artist):return .90
        aliases=[norm(a) for a in e.MOOD_ALIASES.get(mood,[])]
        hits=sum(1 for a in aliases if a and a in title_artist)
        return .86 if hits>=2 else .78 if hits==1 else .52
    ...
    def choose_mood(state,hour=None):
        ...
        recent=list(state.get('recent_moods',[]))[-3:]; old=list(state.get('preview_moods',[]))[-6:]
        ...
        counts={m:recent.count(m)+old.count(m)+preview_memory.count(m) for m in choices}; low=min(counts.values())
    ...
    def pick_music(tracks,mood,state,preview=False):
        ...
        # Tier 1: fresh + strong match.
        fresh=[t for t in tracks if t.get('file_id') not in recent and t.get('file_id') not in preview_ids]
        strong=[t for t in fresh if music_score(t,mood)>=.72]
        if strong:return random.choice(strong[:12])
        # Tier 2: fresh, even if the semantic match is weaker.
        if fresh:return random.choice(sorted(fresh,key=lambda t:music_score(t,mood),reverse=True)[:12])
        # Tier 3: if diversity exhausted the good tracks, reuse a strong match.
        reusable_strong=[t for t in tracks if t.get('file_id') not in recent and music_score(t,mood)>=.72]
        if reusable_strong:return random.choice(sorted(reusable_strong,key=lambda t:music_score(t,mood),reverse=True)[:12])
        # Tier 4: last resort. Better a valid track than no package, but production gate rejects it.
        return random.choice(sorted(tracks,key=lambda t:music_score(t,mood),reverse=True)[:12])

    def build(preview=False):
        state=bot.load_state(); lib=bot.load_library(); best=None; gate=max(82,bot.QUALITY_THRESHOLD)
        for _ in range(max(18,bot.MAX_ATTEMPTS)):
            ...
            if score['overall']>=gate and score['image']>=72 and score['music']>=72 and score['coherence']>=78:break
        if preview:
            if best:
        ...
        sc=best['score']
        if not(sc['overall']>=gate and sc['image']>=72 and sc['music']>=72 and sc['coherence']>=78):return None
        return best
