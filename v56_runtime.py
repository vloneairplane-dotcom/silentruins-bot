...
def main():
    ...
    # Scene vocabulary is deliberately stricter than the broad mood vocabulary.
    SCENES = {
    ...
    def profile(t):
        ...
        if isinstance(old, dict) and all(m in old for m in e.MOODS):
            return old
        tag = norm(t.get("mood"))
        text = norm(f"{t.get('title', '')} {t.get('performer', '')} {tag}")
        ...
        for phrase, vals in PHRASES.items():
            if phrase in text:
                for m, v in vals.items():
                    p[m] = max(p[m], v)
        for m in e.MOODS:
            if tag == m or tag == norm(e.MOODS[m]["fa"]):
                p[m] = 1.0
    ...
    def profile_library(lib):
        ...
        for t in lib.get("tracks", []):
            old = t.get("mood_profile")
            profile(t)
            if old != t.get("mood_profile"):
                changed = True
        if changed:
            bot.save_library(lib)
        return lib

    def music_score(t, mood):
        return float(profile(t).get(mood, .42)) if t else 0.0
    ...
    def choose_mood(state, hour=None):
        ...
        if h >= 22 or h < 5:
            ...
            pool = ["tired", "love", "rain", "lonely", "ruins"]
        ...
        # Soft balancing: frequently used moods are penalized instead of simply
        # being blocked, so all moods can still appear naturally.
        weights = []
        for m in pool:
    ...
    def visual_score(photo, mood):
        ...
        # Portrait imagery is preferred for the Telegram cinematic card.
    ...
    def image_score(photo, mood):
        # Keep the engine's public image_score compatible while making it scene-aware.
        base = e.image_score(photo, mood) if photo else 0.0
        scene = visual_score(photo, mood)
        return max(0.0, min(1.0, .35 * base + .65 * scene))

    def quality(mood, caption, track, photo):
        ts = e.caption_score(caption, mood)
        ms = music_score(track, mood)
        im = image_score(photo, mood)
        co = max(0, min(1, .10 * ts + .52 * ms + .38 * im))
        overall = ts * .24 + im * .30 + ms * .34 + co * .12
        return {
            "text": round(ts * 100), "image": round(im * 100), "music": round(ms * 100),
            "coherence": round(co * 100), "overall": round(overall * 100),
        }

    e.choose_mood = choose_mood
    e.music_score = music_score
    e.image_score = image_score
    ...
    def pick_music(tracks, mood, state, preview):
        ...
        seen = set(state.get("preview_track_ids", [])[-25:]) if preview else set()
        ...
        if strong:
            return random.choice(strong[:12])
        ...
        if reusable:
            return random.choice(reusable[:12])
    ...
    def build(preview=False):
        ...
        for _ in range(max(30, bot.MAX_ATTEMPTS)):
            ...
            # Strong package: don't waste Pexels requests once all hard gates pass.
        ...
        if not best:
            return None
        sc = best["score"]
        # A weak visual is never allowed into Preview or production.
    ...
    async def preview(update, context):
        ...
        if p.get("photo"):
            try:
                ...
                b = io.BytesIO(data); b.name = "preview.jpg"
                await target.reply_photo(photo=b, caption=cap)
            except Exception:
                await target.reply_text(cap)
        else:
            await target.reply_text(cap)
        if p.get("track"):
            t = p["track"]
            await target.reply_audio(audio=t["file_id"], caption=bot._audio_caption(t), title=t.get("title"), performer=t.get("performer"))
